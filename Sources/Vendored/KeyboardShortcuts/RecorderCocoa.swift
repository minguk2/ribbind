	...
	public final class RecorderCocoa: NSSearchField, NSSearchFieldDelegate {
		...
		*/
		...
		private func endRecording() {
			...
			// Re-register Carbon hotkeys that becomeFirstResponder soft-
			// unregistered, so global combos work again after recording ends.
		...
		override public func becomeFirstResponder() -> Bool {
			...
			// Unregister Carbon hotkeys while recording. Carbon's hotkey
			// handler consumes the event (returns noErr) before the
			// Recorder's NSEvent local monitor sees it. Without this step,
			// pressing a combo that's ALREADY registered (e.g. ⇧⌘2 bound to
			// word.FormatPainter while recording for powerpoint.FormatPainter)
			// silently fails — Carbon eats the keystroke and the recorder
			// UI never updates. `softUnregisterAll` keeps the registration
			// metadata so `softRegisterAll` in `endRecording` restores them
			// atomically when the recorder loses focus.
