...
enum CarbonKeyboardShortcuts {
	...
	static func register(
		...
		guard
			...
			NSLog("[CarbonKS] Error registering hotkey %@: err=%d", "\(shortcut)", registerError)
			return
		}
		NSLog("[CarbonKS] Registered hotkey %@ (carbonKC=%d, carbonMods=0x%x, id=%d)",
		      "\(shortcut)", shortcut.carbonKeyCode, shortcut.carbonModifiers, hotKeyId)
