...
**2단계. 아래 블록 실행.** 처음 설치할 때나 업데이트할 때나 같은 명령입니다. 기존 소스 클론을 모두 지우고 GitHub 에서 최신을 다시 받아 빌드한 뒤 `/Applications/` 에 설치합니다:
...
**업데이트하려면?** README 위쪽 설치 블록을 그대로 다시 실행합니다. 기존 소스 클론을 지우고 최신 원격에서 다시 받아 빌드하므로, 실행 중인 앱과 `/Applications/Ribbind.app` 이 한 번에 새 버전으로 교체됩니다.
