...
**3단계. `Ribbind.app` 을 Applications 단축키 위로 드래그합니다.** 설치 끝.

설치 후 Finder 사이드바에서 Ribbind DMG 를 우클릭 → 추출 (Eject) 합니다.

### 첫 실행 (Gatekeeper 1회 우회)

Ribbind 은 ad-hoc 서명만 사용 (유료 Apple Developer 계정 불필요) 이므로 macOS 가 첫 실행 때 한 번 확인을 요청합니다:

1. Finder 에서 `/Applications` 폴더를 열고 **`Ribbind.app` 을 더블클릭** 합니다.
2. macOS 가 *"Ribbind"을(를) 열 수 없습니다 — Apple에서 악성 소프트웨어가 포함되어 있는지 확인할 수 없습니다.* 다이얼로그를 띄웁니다. **완료** 를 클릭합니다.
3. **시스템 설정 → 개인 정보 보호 및 보안** 을 엽니다.
4. 아래로 스크롤하면 *"Ribbind"이(가) 차단됨* 항목이 보입니다. 옆의 **열기** (혹은 **Open Anyway**) 버튼을 누릅니다.
5. Touch ID 또는 비밀번호로 인증합니다. 다시 **열기** 를 누르면 Ribbind 이 실행됩니다.

이 단계는 1회만 필요합니다. 이후로는 `Ribbind.app` 을 더블클릭하면 바로 실행됩니다. Ribbind 은 메뉴바에서 동작하며 Dock 아이콘은 표시되지 않습니다. 설정창은 메뉴바 아이콘에서 열거나 `⌘,` 단축키로 엽니다.

### 권한 부여

시스템 다이얼로그가 처음 뜰 때 한 번씩 클릭하면 됩니다:

- **Accessibility** — 단축키를 Word / PowerPoint / Chrome 보다 먼저 가로채기 위해 필요합니다.
- **Automation** (Word / PowerPoint / Chrome) — Apple Events 로 효과를 적용하기 위해 필요합니다. 각 앱에서 첫 단축키를 누를 때 한 번씩 묻습니다.

### 업데이트

새 릴리즈의 DMG 를 받아서 새 `Ribbind.app` 을 Applications 단축키로 드래그 (기존 것을 덮어쓰기) 한 다음, **시스템 설정 → 개인 정보 보호 및 보안 → 열기** 단계를 1회 다시 거칩니다. 단축키 바인딩, picker 설정값, Accessibility 권한은 그대로 유지됩니다 (다만 코드 서명이 릴리즈마다 바뀌므로 시스템 설정의 Accessibility 목록에서 Ribbind 을 제거 후 다시 추가해야 할 수 있습니다).
...
- **Format**: Format Painter, Highlight 1/2/3 (이름 있는 색상), Font Color 1/2/3 (RGB), Font Family, Paste with Format (텍스트 전용, 바인딩별 타입 picker)
- **Picture**: Crop, Lock Aspect Ratio
...
**이미지 클립보드 상태에서 Paste with Format 단축키를 눌렀을 때.** picker (Unformatted / Match Formatting 등) 는 **텍스트 클립보드에만** 적용됩니다. 클립보드에 이미지/파일이 들어있으면 Ribbind 이 자동으로 평범한 Paste 로 fallback 하므로, 이미지/파일은 정상적으로 붙여넣어집니다.

**"Ribbind 을(를) 열 수 없습니다" 같은 다이얼로그가 뜨는 경우.** 첫 실행 때 한 번만 나오는 Gatekeeper 안내입니다. 다이얼로그의 **완료** 를 누른 뒤, **시스템 설정 → 개인 정보 보호 및 보안** 으로 가서 아래로 스크롤합니다. *"Ribbind"이(가) 차단됨* 항목 옆의 **열기** (Open Anyway) 버튼을 누르고 인증한 다음 다시 **열기** 를 확인합니다. 한 번 허용된 이후로는 더블클릭으로 정상 실행됩니다. (macOS Sequoia 15+ 부터는 이 경로만 사용 가능합니다 — Finder 우클릭 메뉴에 있던 옛 단축 옵션이 제거되었습니다.)
...
**Apple Developer 유료 계정이 필요한가요?** 필요하지 않습니다. 배포되는 `.app` 은 ad-hoc 서명만 사용합니다 (무료, 유료 계정 불필요). 그 대가로 위에서 안내한 첫 실행 시 *시스템 설정 → 개인 정보 보호 및 보안 → 열기* 단계가 1회 필요합니다.
...
**업데이트하려면?** 두 가지 방법 모두 가능합니다. (1) 새 릴리즈의 DMG 를 받아 새 `Ribbind.app` 을 Applications 단축키로 드래그 (기존 것을 덮어쓰기) 한 다음, *시스템 설정 → 개인 정보 보호 및 보안 → 열기* 단계를 1회 다시 거칩니다. (2) 또는 위 *소스에서 직접 빌드* 블록을 다시 실행하면 최신 원격을 받아 한 번에 교체합니다. 어느 쪽이든 단축키 바인딩과 picker 설정값은 유지됩니다.
